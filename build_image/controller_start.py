    ...
    for i, (name, row) in enumerate(mtypes.iterrows()):
        ...
        if len(tmp) == 3:
            ...
            nodes.append('NodeName={name} CPUs={cpu} RealMemory={mem} State=CLOUD Weight={cpu}'.format(
                name=worker_name,
                cpu=row.CPUS,
                mem=(993 * row.MEMORY_GB)-400, # 993 is 97% of 1024. Accounts for system overhead in memory
            ))
...
def main():
    ...
    slurm_conf['CONTROLLER HOSTNAME'] = cluster_name
    ...
    slurmdbd_conf['CONTROLLER HOSTNAME'] = cluster_name
